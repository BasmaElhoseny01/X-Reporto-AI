...
from src.object_detector_copy.models.object_detector_paper.custom_roi_heads import CustomRoIHeads
from src.object_detector_copy.models.object_detector_paper.custom_rpn import CustomRegionProposalNetwork
from src.object_detector_copy.models.object_detector_paper.image_list import ImageList


class ObjectDetectorOriginal(nn.Module):
    """
    ...
    def forward(self, images: Tensor, targets: Optional[List[Dict[str, Tensor]]] = None):
        ...
        # with torch.no_grad():
            ...
            # for idx, layer in enumerate(self.backbone):
                ...
                # print(x)
                # if(idx==7):
                #     print(f"In: {x.min()}, {x.max()}, {torch.isnan(x).any()}")
                #     print(x.shape,"...................")
                #     for layer_1 in layer:
                #         x=layer_1(x)
                #         print(f"Output after layer {idx}: {x.min()}, {x.max()}, {torch.isnan(x).any()}")
        ...
        # print("roi_heads_output",roi_heads_output)
